...
app.post('/process-video', async (req, res) => { //this endpoint not going to be invoked by any user but by the cloud pub/sub message, everytime a file is uploaded to raw video bucket
    ...
    // this is important as if we take too long to process video, pub/sub since it didnt recieve ack, will call this function again, thus we dont want to process if pub/sub calls this service for the same video the second time
